...
function deleteNoneRows(tr) {
	for (var i = 0; i < tr.length; i++) {
		...
		if (td) {
			if (td.innerHTML.indexOf('Stay Tuned!') > -1) {
				table.deleteRow(i);
			}
		}
	}
	...
	if (checkId(id)) {
		...
		if (id == 'Undo') {
			...
			for (var i = 1; i < tr.length; i++) {
				td = tr[i].getElementsByTagName('td')[1].getElementsByClassName('tableTagsContainer')[0];
				if (td) {
					if (td.children.length == 0) {
						tr[i].style.display = 'none';
					} else {
						for (var c = 0; c < td.children.length; c++) {
							if (td.children[c].title.toUpperCase().search(id.toUpperCase()) != -1) {
			...
			if (noneSelected) {
				...
				c1.innerHTML = 'No ' + id + ' projects yet, Stay Tuned!';
